...
class PopularCollectionViewCell: UICollectionViewCell, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    let popularVideos: [VideoData] = {
        ...
        video1.Image = UIImage(named: "romance-image")
        video1.tag = ["Workplace", "Korean","Sentimental"]
        
        let video2 = VideoData()
        video2.Image = UIImage(named: "the-alienist")
        video2.tag = ["Mascuud", "Hussien", "Musse"]
        
        let video3 = VideoData()
        video3.Image = UIImage(named: "designated-survior")
        video3.tag = ["Mascuud", "Hussien", "Musse"]
        
        let video4 = VideoData()
        video4.Image = UIImage(named: "crown")
        video4.tag = ["Mascuud", "Hussien", "Musse"]
...
class PopularCustomCell: UICollectionViewCell {
    ...
    var popularVideos: VideoData? {
        didSet {
            videoImage.image = popularVideos?.Image
            if let videoTitle = popularVideos?.tag{
                firstImageLabel.attributedText = NSAttributedString(string:  videoTitle[0], attributes: [NSAttributedString.Key.foregroundColor: UIColor.white, .font: UIFont.boldSystemFont(ofSize: 14)])
                firstImageLabel.attributedText = NSAttributedString(string:  videoTitle[1], attributes: [NSAttributedString.Key.foregroundColor: UIColor.white,.font: UIFont.boldSystemFont(ofSize: 14)])
                thirdImageLabel.text = popularVideos?.tag?[2]
            }
